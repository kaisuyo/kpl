...
void compileBlock3(void) {
  Object** varObj = (Object**)malloc(sizeof(Object)*100);
  Type* varType;
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      ...
      do{
        ...
        checkFreshIdent(currentToken->string);
        varObj[i++] = createVariableObject(currentToken->string);
      ...
      int j;
      for (j = 0; j < i; j++) {
        varObj[j]->varAttrs->type = varType;
        declareObject(varObj[j]);
      }
      eat(SB_SEMICOLON);
    } while (lookAhead->tokenType == TK_IDENT);
...
void compileSuperAssignSt(void) {
  Type** LType = (Type**)malloc(sizeof(Type)*100);
  Type** RType = (Type**)malloc(sizeof(Type)*100);
  int variableLTotal = 0;
  int variableRTotal = 0;

  int ln = lookAhead->lineNo, col = lookAhead->colNo;

  while(1) {
    LType[variableLTotal++] = compileLValue();
  ...
  while(1) {
    ...
    if (lookAhead->tokenType == SB_COMMA) {
      ...
      eat(SB_QUESTION);
      Type* t3 = compileExpression();
      eat(SB_COLON);
      Type* t4 = compileExpression();
      checkTypeEquality(t3, t4);
      RType[variableRTotal-1] = t3;
      if (lookAhead->tokenType == SB_COMMA) {
        eat(SB_COMMA);
      } else {
        break;
  ...
  if (variableLTotal != variableRTotal) {
    error(ERR_INVALID_PARAMETER, ln, col);
  }
