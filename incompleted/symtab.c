...
int compareType(Type* type1, Type* type2) {
	...
	if (type1->typeClass == TP_ARRAY && type2->typeClass == TP_ARRAY) {
		if (type1->elementType->typeClass == TP_CHAR && type2->elementType->typeClass == TP_CHAR) {
			if (type1->arraySize >= type2->arraySize) return -1;
		}
	}
